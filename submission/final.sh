...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r ".vout[] | select(.scriptPubKey.address == \"$PAYMENT_ADDRESS\") | .value | @text")
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r ".vout[] | select(.scriptPubKey.address == \"$CHANGE_ADDRESS\") | .value | @text")
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r ".vout | map(.scriptPubKey.address == \"$CHANGE_ADDRESS\") | index(true)")
